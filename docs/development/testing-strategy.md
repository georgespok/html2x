...
This page explains test ownership and durable focused commands. Html2x tests
should describe observable behavior: computed styles, geometry, fragments,
diagnostics, PDF validity, extracted text, and public API results.
...
- `CssStyleComputer`.
- `StyleTreeBuilder`.
- Default stylesheet application.
...
architecture guardrails. It may inspect contract boundary facts, but it must not
...
- Explain the behavior change in the change summary.
- Update nearby tests when the change crosses layers.

## Focused Commands

Use [Build, Run, And Test](build-run-test.md) for full restore, build, and test
