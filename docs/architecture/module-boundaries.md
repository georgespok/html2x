...
| --- | --- | --- | --- | --- |
| Public Facade | `Html2x` | Consumer configuration | `HtmlConverterOptions`, public option groups, public result, and mapping into stage-owned settings | Layout algorithms, renderer algorithms, dependency adapters, documents, fragments, diagnostics runtime internals |
...
| Pagination | `Html2x.LayoutEngine.Pagination` | Render model block fragments and `PaginationOptions` | `PaginationResult`, final `HtmlLayout`, translated fragment clones, page audit facts, placement audit facts | Source fragment mutation, mutable boxes, style facts, geometry engines, fragment tree building, parser state, renderer state |
| Paint | `Html2x.Renderers.Pdf` | `HtmlLayout`, internal `PdfRenderSettings`, diagnostics sink | Paint commands and PDF bytes | Layout pages, fragments, boxes, styles, parser objects, public converter options |
...
`PaginationOptions`, and internal `PdfRenderSettings`.
...
Render model facts can be shared by layout, text, fragment tree building,
pagination, renderers, and options without introducing dependency adapter
dependencies.
...
`Html2x.LayoutEngine.Geometry.Models`. Geometry implementation modules are
organized by owner namespaces such as `Construction`, `BlockFlow`,
...
- Fragment tree building copies published facts into render model fragments.
...
table fragment tree building. Renderers do not reference fragment tree building.
...
`LayoutPipeline` calls style first, passes the resulting `StyleTree` to
geometry, builds `FragmentTree` from `PublishedLayoutTree`, calls pagination,
and returns `PaginationResult.Layout`. If composition needs more data, the
owning stage must publish that data through its handoff contract.
