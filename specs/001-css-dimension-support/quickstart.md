...
2. **Run Layout Tests**
  ```powershell
  dotnet test Html2x.sln -c Release --filter "FullyQualifiedName~Html2x.LayoutEngine.Test && Category=Dimensions"
  ```
   - Covers style parsing, unit normalization, and conflicting constraint resolution.
3. **Run Renderer Regression**
  ```powershell
  dotnet test Html2x.sln -c Release --filter "Html2x.Renderers.Pdf.Test&&Category=BorderedBlocks"
  ```
...
4. **Smoke-Test via Console**
  ...
  dotnet run --project src/Tests/Html2x.TestConsole/Html2x.TestConsole.csproj -- `
    --input src/Tests/Html2x.TestConsole/html/width-height/grid.html `
