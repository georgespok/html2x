...
internal static class ListMarkerPolicy
{
    public static InlineBox? CreateMarker(BoxNode listContainer, BlockBox listItem)
    {
        ...
        return CreateMarkerInlineBox(markerText, listItem);
    }
    ...
    private static string ResolveMarkerText(BoxNode listContainer, BoxNode listItem)
    {
        ...
        if (string.Equals(tag, HtmlCssVocabulary.HtmlTags.Ul, StringComparison.OrdinalIgnoreCase))
        {
            return "\u2022 ";
        }

        if (string.Equals(tag, HtmlCssVocabulary.HtmlTags.Ol, StringComparison.OrdinalIgnoreCase))
        {
    ...
    private static InlineBox CreateMarkerInlineBox(string markerText, BlockBox listItem) =>
        new(BoxRole.Inline)
        {
