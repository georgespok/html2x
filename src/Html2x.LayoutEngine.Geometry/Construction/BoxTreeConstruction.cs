...
namespace Html2x.LayoutEngine.Geometry.Construction;
...
internal sealed class BoxTreeConstruction
{
    ...
    private BoxNode BuildNode(StyleNode styleNode, BoxNode? parent, TextNormalizationState textState)
    {
        ...
        {
            BoxTreeNormalization.NormalizeChildrenForBlock(childBlock);
        }
        ...
        GeometrySourceIdentity sourceIdentity)
    {
        ...
        if (HtmlElementRules.IsImage(element))
        {
            return new ImageBox(role)
            {
                ...
                IsAnonymous = isAnonymous,
                SourceIdentity = sourceIdentity,
                Src = element.GetAttribute(HtmlCssVocabulary.HtmlAttributes.Src) ?? string.Empty
            };
        }
    ...
    private BlockBox CreateInlineBlockContentBox(InlineBox inlineBlock, StyleNode styleNode)
    {
        ...
        contentBox.EstablishesInlineBlockFormattingContext = true;
        ...
        TextNormalizationState textState)
    {
        ...
        box.AddChild(BuildNode(childStyleNode, box, childState));
    ...
    private static void AddTextRun(StyleContentNode content, BoxNode box, TextNormalizationState textState)
    {
        ...
        var normalized = StringNormalizer.CollapseNormalWhiteSpace(text, textState);
        if (string.IsNullOrEmpty(normalized))
        {
            return;
        }

        box.AddChild(new InlineBox(BoxRole.Inline)
        {
    ...
    private static void AddListMarker(StyledElementFacts element, BoxNode? parent, BoxNode box)
    {
        ...
        listItem.MarkerOffset = HtmlCssVocabulary.Defaults.ListMarkerOffsetPt;
        box.InsertChild(0, marker);
    }
    ...
    private static bool IsFloating(ComputedStyle style) =>
        string.Equals(style.FloatDirection, HtmlCssVocabulary.CssValues.Left, StringComparison.OrdinalIgnoreCase) ||
        string.Equals(style.FloatDirection, HtmlCssVocabulary.CssValues.Right, StringComparison.OrdinalIgnoreCase);
}
