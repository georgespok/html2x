...
/// <summary>
///     Normalizes constructed block children into the canonical box sequence expected by layout.
/// </summary>
internal static class BoxTreeNormalization
{
    public static void NormalizeChildrenForBlock(BlockBox parent)
    {
        if (parent.IsAnonymous && !parent.EstablishesInlineBlockFormattingContext)
        {
            return;
        }
        ...
        parent.ReplaceChildren(normalized);
    }

    private static BoxNode? CreateInlineBlockBoundaryNode(BlockBox parent, BoxNode child)
    {
        if (!parent.EstablishesInlineBlockFormattingContext ||
    ...
    private static BlockBox CreateAnonymousBlock(BoxNode parent, List<InlineBox> inlines)
    {
        var anon = new BlockBox(BoxRole.Block)
        {
            ...
            TextAlign = parent.Style.TextAlign ?? HtmlCssVocabulary.Defaults.TextAlign,
        ...
        BlockBox sourceContentBox)
    {
        var boundary = new InlineBlockBoundaryBox(sourceInline, sourceContentBox)
        {
            ...
            EstablishesInlineBlockFormattingContext = sourceContentBox.EstablishesInlineBlockFormattingContext,
            SourceIdentity = GeometrySourceIdentity
        ...
        {
            boundary.AddChild(child.CloneForParent(boundary));
        }
