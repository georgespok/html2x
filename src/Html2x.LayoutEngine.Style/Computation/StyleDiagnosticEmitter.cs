...
internal static class StyleDiagnosticEmitter
{
    ...
    public static void EmitUnsupportedElement(
        IDiagnosticsSink? diagnosticsSink,
        IElement element,
        string reason)
    {
        diagnosticsSink?.Emit(new(
            ...
            StyleDiagnosticNames.Events.UnsupportedElement,
            DiagnosticSeverity.Warning,
            reason,
            CreateElementDiagnosticContext(element),
            DiagnosticFields.Create(
                DiagnosticFields.Field(StyleDiagnosticNames.Fields.TagName, element.LocalName),
                DiagnosticFields.Field(StyleDiagnosticNames.Fields.Decision, StyleDiagnosticNames.Decisions.Unsupported),
    ...
    private static DiagnosticContext CreateDiagnosticContext(
        ...
        new(
            ...
            element.GetAttribute(HtmlCssVocabulary.HtmlAttributes.Style));

    private static DiagnosticContext CreateElementDiagnosticContext(IElement element) =>
        new(
            CreateSelector(element),
            CreateElementIdentity(element),
            null,
