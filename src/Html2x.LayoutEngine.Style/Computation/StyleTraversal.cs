...
internal sealed class StyleTraversal
{
        ...
        IDiagnosticsSink? diagnosticsSink = null, CancellationToken cancellationToken = default)
    {
        ...
        return BuildNode(root, rootFacts, rootIdentity, null, styleFactory, state, diagnosticsSink, cancellationToken);
    }

    private StyleNode BuildNode(IElement element,
        StyledElementFacts facts,
        StyleSourceIdentity identity,
        ComputedStyle? parent,
        Func<IElement, ComputedStyle?, ComputedStyle> styleFactory,
        TraversalIdentityState state,
        IDiagnosticsSink? diagnosticsSink, CancellationToken cancellationToken)
    {
        ...
        foreach (var child in element.ChildNodes)
        {
            ...
            {
                StyleDiagnosticEmitter.EmitUnsupportedElement(
                    diagnosticsSink,
                    childElement,
                    $"Unsupported element '{childElement.LocalName}' was flattened.");
    ...
    private static StyledElementFacts CreateFacts(IElement element)
    {
        ...
        return new(
            element.TagName,
            element.LocalName,
            element.GetAttribute(HtmlCssVocabulary.HtmlAttributes.Id),
            element.GetAttribute(HtmlCssVocabulary.HtmlAttributes.Class),
            attributes);
    }
