...
public class HtmlRenderedToPdfTests(ITestOutputHelper output) : IntegrationTestBase(output)
{
    ...
    private static LayoutPageSnapshot GetLayoutPageSnapshot(Html2PdfResult result)
    {
        var endLayoutBuild =
            result.DiagnosticsReport!.Records.FirstOrDefault(x => x is
                { Stage: "LayoutBuild", Name: "stage/succeeded" });
    ...
    private static (float ContainerHeight, float FirstLineHeight, float SecondLineHeight, float LineDeltaY)
        ExtractTwoLineMetrics(
    ...
    private static void AssertIncreasingOrder(IReadOnlyList<string> orderedTexts,
