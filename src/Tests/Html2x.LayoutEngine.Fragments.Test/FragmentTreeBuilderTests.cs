...
public sealed class FragmentTreeBuilderTests
{
    [Fact]
    public void Build_EmptyLayout_ReturnsNoRootFragments()
    {
        var fragments = new FragmentTreeBuilder().Build(PublishedLayoutFragmentTestBuilder.Tree());
    ...
    public void Build_TextRuns_PreservesResolvedFontFromPublishedLayout()
    {
        ...
        var fragments = new FragmentTreeBuilder().Build(PublishedLayoutFragmentTestBuilder.Tree(block));
    ...
    public void Build_ImageBlock_EmitsImageFragmentWithImageFacts()
    {
        ...
        imageFragment.IsOversized.ShouldBeFalse();
    ...
    [Fact]
    public void Build_BlockWithInlineFlowAndRule_AppendsRuleAfterInlineFlow()
    {
        var segment = PublishedLayoutFragmentTestBuilder.Segment(
            PublishedLayoutFragmentTestBuilder.TextItem(0, "before"));
        var block = PublishedLayoutFragmentTestBuilder.Block(
            inlineLayout: PublishedLayoutFragmentTestBuilder.InlineLayout(segment),
            rule: new(),
            flow:
            [
                new PublishedInlineFlowSegmentItem(0, segment)
        ...
        fragments.Blocks.ShouldHaveSingleItem()
            .Children
            .Select(static fragment => fragment.GetType())
            .ShouldBe([
    ...
    [Fact]
    public void Build_BlockWithChildFlowAndRule_AppendsRuleAfterChildBlock()
    {
        var childSegment = PublishedLayoutFragmentTestBuilder.Segment(
            PublishedLayoutFragmentTestBuilder.TextItem(0, "child"));
        var child = PublishedLayoutFragmentTestBuilder.Block(
        ...
        var root = PublishedLayoutFragmentTestBuilder.Block(
            ...
            flow:
            [
                new PublishedChildBlockItem(0, child)
            ]);

        var fragments = Build(PublishedLayoutFragmentTestBuilder.Tree(root));

        var rootFragment = fragments.Blocks.ShouldHaveSingleItem();
        rootFragment.Children
            .Select(static fragment => fragment.GetType())
            .ShouldBe([
                typeof(BlockFragment),
                typeof(RuleFragment)
            ]);
        rootFragment.Children[0]
    ...
    private static FragmentTree Build(PublishedLayoutTree layout) => new FragmentTreeBuilder().Build(layout);
}
