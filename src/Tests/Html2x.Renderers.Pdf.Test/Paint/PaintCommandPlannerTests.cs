...
public sealed class PaintCommandPlannerTests
{
    private static readonly BorderEdges Border = BorderEdges.Uniform(
        new(1f, ColorRgba.Black, BorderLineStyle.Solid));

    [Fact]
    public void Resolve_BlockWithTextImageAndRule_PreservesCurrentTraversalOrder()
    {
        var line = CreateLine(2, "alpha", new(12f, 14f, 60f, 12f));
        var image = new ImageFragment
        {
            FragmentId = 3,
            Rect = new(20f, 30f, 40f, 20f),
            ContentRect = new(21f, 31f, 38f, 18f),
            Src = "image.png",
            Style = new(Borders: Border)
        };
        var rule = new RuleFragment
        {
            FragmentId = 4,
            Rect = new(10f, 60f, 90f, 2f),
            Style = new(Borders: Border)
        };
        var block = new BlockFragment([line, image, rule])
        {
            FragmentId = 1,
            Rect = new(10f, 10f, 100f, 70f),
            Style = new(
                new ColorRgba(240, 240, 240, 255),
                Border)
        };

        var commands = new PaintCommandPlanner().Resolve(CreatePage(block));
    ...
    {
        var line = CreateLine(4, "cell", new(14f, 16f, 40f, 12f));
        var cell = new TableCellFragment([line])
        {
            FragmentId = 3,
            Rect = new(12f, 12f, 80f, 30f),
            Style = new(
                new ColorRgba(220, 240, 220, 255),
                Border)
        };
        var row = new TableRowFragment([cell])
        {
            FragmentId = 2,
            Rect = new(10f, 10f, 90f, 34f),
            Style = new(
                new ColorRgba(220, 220, 240, 255),
                Border)
        };
        var table = new TableFragment([row])
        {
            FragmentId = 1,
            Rect = new(8f, 8f, 100f, 40f),
            Style = new(
                new ColorRgba(240, 240, 240, 255),
                Border)
        };

        var commands = new PaintCommandPlanner().Resolve(CreatePage(table));
    ...
    public void Resolve_OutOfOrderZValues_PaintsLowerZOrderFirst()
    {
        var first = new BlockFragment
        {
            FragmentId = 10,
            ZOrder = 20,
            Rect = new(0f, 0f, 10f, 10f),
            Style = new(new ColorRgba(10, 10, 10, 255))
        };
        var second = new BlockFragment
        {
            FragmentId = 20,
            ZOrder = 1,
            Rect = new(20f, 0f, 10f, 10f),
            Style = new(new ColorRgba(20, 20, 20, 255))
        };

        var commands = new PaintCommandPlanner().Resolve(CreatePage(first, second));
    ...
    {
        var line = CreateLine(2, "stable", new(12f, 14f, 60f, 12f));
        var block = new BlockFragment([line])
        {
            FragmentId = 1,
            Rect = new(10f, 10f, 100f, 30f),
            Style = new(new ColorRgba(240, 240, 240, 255))
        };
        ...
        _ = new PaintCommandPlanner().Resolve(CreatePage(block));
    ...
    public void Resolve_UnknownFragmentType_ThrowsWithClosedFragmentSetGuidance()
    {
        ...
        var exception = Should.Throw<NotSupportedException>(() =>
            new PaintCommandPlanner().Resolve(CreatePage(fragment)));
