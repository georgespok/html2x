...
public sealed class PaintCommandBuilderTests
{
    private static readonly BorderEdges Border = BorderEdges.Uniform(
        new BorderSide(1f, ColorRgba.Black, BorderLineStyle.Solid));

    [Fact]
    public void Build_BlockWithTextImageAndRule_PreservesCurrentTraversalOrder()
    {
        var line = CreateLine(2, "alpha", new RectangleF(12f, 14f, 60f, 12f));
        var image = new ImageFragment
        {
            FragmentId = 3,
            Rect = new RectangleF(20f, 30f, 40f, 20f),
            ContentRect = new RectangleF(21f, 31f, 38f, 18f),
            Src = "image.png",
        ...
        {
            FragmentId = 4,
            Rect = new RectangleF(10f, 60f, 90f, 2f),
            Style = new VisualStyle(Borders: Border)
        };
        var block = new BlockFragment([line, image, rule])
        {
            FragmentId = 1,
            Rect = new RectangleF(10f, 10f, 100f, 70f),
            Style = new VisualStyle(
                BackgroundColor: new ColorRgba(240, 240, 240, 255),
                Borders: Border)
        };

        var commands = new PaintCommandBuilder().Build(CreatePage(block));
    ...
    [Fact]
    public void Build_Table_PreservesCurrentLayeringOrder()
    {
        var line = CreateLine(4, "cell", new RectangleF(14f, 16f, 40f, 12f));
        var cell = new TableCellFragment([line])
        {
            FragmentId = 3,
            Rect = new RectangleF(12f, 12f, 80f, 30f),
            Style = new VisualStyle(
        ...
        {
            FragmentId = 2,
            Rect = new RectangleF(10f, 10f, 90f, 34f),
            Style = new VisualStyle(
        ...
        {
            FragmentId = 1,
            Rect = new RectangleF(8f, 8f, 100f, 40f),
            Style = new VisualStyle(
                BackgroundColor: new ColorRgba(240, 240, 240, 255),
                Borders: Border)
        };

        var commands = new PaintCommandBuilder().Build(CreatePage(table));
    ...
    [Fact]
    public void Build_OutOfOrderZValues_PreservesTraversalAndCarriesZOrderMetadata()
    {
        var first = new BlockFragment
        {
            FragmentId = 10,
            ZOrder = 20,
            Rect = new RectangleF(0f, 0f, 10f, 10f),
        ...
        var second = new BlockFragment
        {
            FragmentId = 20,
            ZOrder = 1,
            Rect = new RectangleF(20f, 0f, 10f, 10f),
            Style = new VisualStyle(BackgroundColor: new ColorRgba(20, 20, 20, 255))
        };

        var commands = new PaintCommandBuilder().Build(CreatePage(first, second));

        commands.Skip(1).Select(static command => command.SourceFragmentId).ShouldBe([10, 20]);
        commands.Skip(1).Select(static command => command.ZOrder).ShouldBe([20, 1]);
    }

    [Fact]
    public void Build_Fragments_DoesNotMutateSourceFragments()
    {
        var line = CreateLine(2, "stable", new RectangleF(12f, 14f, 60f, 12f));
        var block = new BlockFragment([line])
        {
            FragmentId = 1,
            Rect = new RectangleF(10f, 10f, 100f, 30f),
        ...
        _ = new PaintCommandBuilder().Build(CreatePage(block));
    ...
    private static LineBoxFragment CreateLine(int fragmentId, string text, RectangleF rect)
    {
